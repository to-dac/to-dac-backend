...
- 숫자 값 생성:
  · 논리적으로 같을 필요가 없는 독립 숫자 항목은 서로 다른 값으로 생성한다(모든 숫자를 같은 값으로 채우지 마라).
  · 단, 합계 = 구성요소의 합처럼 논리적으로 일치해야 하는 값은 일관되게 맞춘다.
  · 개수·수량 항목(예: 나무 본수·그루, 세대·가구·호수, 주차대수 등)은 현실적인 범위로,
    근거가 없으면 1000 미만의 값으로 생성한다.
